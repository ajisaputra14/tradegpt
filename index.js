...
const logger = {
  ...
  banner: () => {
    ...
    console.log(`██║  ██║ ███████╗ ██║███████╗ ██╗  ███████║`);
    console.log(`╚═╝  ╚═╝ ╚══════╝ ╚═╝╚══════╝ ╚═╝  ╚══════╝`);
    console.log(`               TradeGPT - 0G               `);
    console.log(`======https://github.com/ajisaputra14======${colors.reset}\n`);
  }
}

// Fungsi untuk delay acak dengan countdown di satu baris
const randomDelay = (min = 31, max = 61) => {
  return new Promise((resolve) => {
    const delaySeconds = Math.floor(Math.random() * (max - min + 1)) + min;
    let elapsed = 0;
    const interval = 5000;

    const message = chalk.gray(`⌛ Waiting for ${delaySeconds}s... (Remaining: ${delaySeconds}s)\r`);
    process.stdout.write(message);

    const tick = () => {
      setTimeout(() => {
        elapsed += interval;
        const remaining = Math.max(0, Math.floor((delaySeconds * 1000 - elapsed) / 1000));
        process.stdout.write(chalk.gray(`⌛ Waiting for ${delaySeconds}s... (Remaining: ${remaining}s)\r`));

        if (elapsed < delaySeconds * 1000) {
          tick();
        } else {
          // Bersihkan baris sebelum lanjut
...
// Gunakan chalk untuk warna teks
  ...
  for (const privateKey of privateKeys) {
    try {
      ...
      for (let i = 0; i < numPrompts; i++) {
        ...
        // Tambahkan delay dengan countdown di tempat
        await randomDelay(31, 61); // Delay antara 31 - 61 detik
      }
    } catch (error) {
